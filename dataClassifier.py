...
if __name__ == '__main__':
    # Read input
    print("10% of data")
    #sys.argv = ['dataClassifier.py', "-c", "perceptron", "-d", "digits", "-t", "500", "-i", "5"]
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "500", "-i", "5"])
    # Run classifier
    runClassifier(args, options)

    print("20% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "1000", "-i", "5"])
    runClassifier(args, options)
    
    print("30% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "1500", "-i", "5"])
    runClassifier(args, options)

    print("40% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "2000", "-i", "5"])
    runClassifier(args, options)

    print("50% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "2500", "-i", "5"])
    runClassifier(args, options)

    print("60% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "3000", "-i", "5"])
    runClassifier(args, options)

    print("70% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "3500", "-i", "5"])
    runClassifier(args, options)
    
    print("80% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "4000", "-i", "5"])
    runClassifier(args, options)
    
    print("90% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "4500", "-i", "5"])
    runClassifier(args, options)

    print("100% of data")
    args, options = readCommand([ "-c", "perceptron", "-d", "digits", "-t", "5000", "-i", "5"])
    runClassifier(args, options)